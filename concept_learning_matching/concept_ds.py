...
class PerceptPairsDataset(InMemoryDataset):
    ...
    def __init__(self, root, name, train=True, transform=None,
        ...
        self.name = name
        super(PerceptPairsDataset, self).__init__(root, transform, pre_transform,
                                             pre_filter)
    ...
    @property
    def raw_dir(self):
        return osp.join(f'{os.getcwd()}', 'correct_percepts')
    ...
    def processed_dir(self):
        dir_to_save = os.path.join(os.getcwd(), 'caches_ds_tempc')
        return dir_to_save
    ...
    def process(self):
        ...
        for file in self.raw_file_names[:30]:
            with open(f'{self.raw_dir}/{file}', "rb") as output_file:
                ...
                nodes, action, reward, _, _, _, nr_proposals, nr_templates, template_labels ,_ ,_ = episodes_data
            for idx in range(len(nodes)-1):
                ...
                x1 = nodes_g1.unsqueeze(0)
                x2 = nodes_g2.unsqueeze(0)
                adj_1 = self.init_adj_matrix(x1)
                adj_2 = self.init_adj_matrix(x2)
                train_y,all_pos = self.process_permutation_matrix(template_labels_g1, template_labels_g2, nodes_g1, nodes_g2, nr_proposals_g1, nr_proposals_g2)
