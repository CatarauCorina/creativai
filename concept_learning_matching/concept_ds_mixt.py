...
class PerceptPairsDatasetMix2(InMemoryDataset):
    ...
    def __init__(self, root, name, train=True, transform=None,
        ...
        self.name = name
        super(PerceptPairsDatasetMix2, self).__init__(root, transform, pre_transform,
                                             pre_filter)
    ...
    @property
    def raw_dir(self):
        return osp.join(f'{os.getcwd()}', 'percepts_short')
    ...
    def processed_dir(self):
        dir_to_save = os.path.join(os.getcwd(), 'caches_ds_mixtc1')
        return dir_to_save
    ...
    def process(self):
        ...
        for file in self.raw_file_names[:30]:
            ...
            for idx in range(len(nodes)-1):
                ...
                matches_temp_roi = self.find_coords_matches(masks_temp_g1, masks_roi_g1, nr_proposals_g1.item(), masks_roi_g2)
    ...
    def find_coords_matches(self, masks_temp_g1, masks_roi_g1, rois_g1, masks_roi_g2):
        matches = []
        ...
        for idx_i, i in enumerate(masks_roi_g1):
            for idx_j, j in enumerate(masks_roi_g2):
                find_overlaps = torch.sum((i * j) == True)
                if find_overlaps.item() > 0:
    ...
    def process_permutation_matrix(self, labels_g1, labels_g2, nodes_g1, nodes_g2, nr_proposals_g1, nr_proposals_g2):
        ...
        permutation_matrix = torch.zeros((nr_nodes_1, nr_nodes_2), dtype=torch.float32)
...
def main():
    ds = PerceptPairsDatasetMix2(os.path.join(os.getcwd(), 'caches_ds_mixtc1'), 'percepts_short')
    print(len(ds))
    return
